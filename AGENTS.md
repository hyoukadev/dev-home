...
- **分层职责** — Containerfile 负责无需网络的基础固化（apt 换源、用户创建、COPY 入口脚本）；entrypoint.sh 负责运行时初始化（apt 安装、GPU 用户态包、nushell、mise、工具、配置），网络操作集中在运行时阶段。

## 文件职责

- `compose.yml` — 容器编排的唯一配置源，`network_mode: host` 使运行时共享宿主机网络；GPU/iGPU 通过 `devices` 占位、`group_add: keep-groups`、`security_opt: label=disable` 支持
...
- `entrypoint.sh` — 内置于镜像（`/usr/local/bin/entrypoint.sh`）。Phase 1（root）：Debian apt 换源 → apt 系统依赖与 GPU 用户态包 → locale-gen → vendor dir → re-exec 为 dev；Phase 2（dev）：mise + 工具 + ohmynushell 配置 + vendor autoloads + oh-my-tmux
...
- `.env` — GITHUB_TOKEN、DEV_HOME_HOST_WORKSPACE、DEV_HOME_GPU/DEV_HOME_*_DEVICE，本地文件，gitignore
...
- GPU/iGPU 支持保持可选：`DEV_HOME_GPU=auto|none|dri|nvidia|amd`；默认自动探测 `/dev/dri`、`/dev/kfd`、`/etc/cdi`、`/var/run/cdi`，无设备时使用 dummy device 保持容器可启动；实际启用 GPU 时 entrypoint 必须保留 Podman `keep-groups` 传入的补充组
